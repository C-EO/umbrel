...
export type DriveLed = 'green' | 'red' | 'none'
...
export function DriveIcon({led = 'none', className}: {led?: DriveLed; className?: string}) {
	return (
		<div className={cn('relative h-11 w-12 shrink-0', className)}>
			...
			{led === 'red' && (
				<span
					className='absolute size-[11%] rounded-full bg-[#FF3434] shadow-[0_0_6px_1px_rgba(255,52,52,0.8)]'
...
export function SsdChip({sizeLabel, className}: {sizeLabel: string; className?: string}) {
	return (
