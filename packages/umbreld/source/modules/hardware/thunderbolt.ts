...
export default class Thunderbolt {
	...
	async #authorize(id: string) {
		...
		if (!device) throw new Error('Thunderbolt device is no longer connected')

		// A UUID identifies a device but does not authenticate it. Unless the
		// kernel reports a known UUID-compatible mode, remove any stored approval
		// before granting access for this connection. The owner will be prompted
		// again after reboot or reconnect.
		const remember = canPersistAuthorization(device.security)
		if (!remember) await this.#forget(device.id)
	...
	async #reconcile() {
		...
		for (const notification of notifications) {
			if (
				(notification.startsWith(THUNDERBOLT_NOTIFICATION_PREFIX) && !activeNotifications.has(notification)) ||
				notification.startsWith(LEGACY_EXTERNAL_GPU_NOTIFICATION_PREFIX)
			) {
